...
for i in ../../Genomes/Branchiopods/*genomic.fa; do
        ...
        for j in {1..5}; do
                ...
                if [ "$j" = 1 ]; then
                        ...
                        varRound="$(( j-1 ))"
                        ln -s ../rnd"$varRound"/"$varSpecie"_auto_rnd"$varRound"
                        python ../../../../TE_Scripts/AutomaticBEE.py --genome "$varSpecie".genomic.fa --lib "$varSpecie"_auto_rnd"$varRound" --out "$varSpecie"_auto_rnd"$j" --blast_identity 70 --blast_query_cov 70 --num_threads 30 --min_Blast_Hits 5;
                fi;
                cd ../;
        done;
        cd ../
